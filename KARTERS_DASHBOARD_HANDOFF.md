...
1. Completion and LOCKING of **Stage 3** (amber) of the still-outstanding **JARVIS Core visual refinement** work (§2, §13) — the "Color + Energy Mapping" stage.
2. Completion and LOCKING of **Stage 4** (whole-Core production-scale visual presence/luminosity) — particle-distribution fix and `frontBoost` calibration, both now locked and committed.
3. **Stage 5** (Reference 1 energy composition) opened at the very end of this session: a diagnosis-only pass (§ "STAGE 5" below) was completed and presented, but **no implementation has happened yet** — this is the actual next open item.

This was **not** Phase 8. Phase 8 ("Performance, QA & Final Polish") remains exactly as described below: not started.

### Git state
- **Checkpoint `178e673`** — "Checkpoint: Stage 3 amber topology-gated production baseline" — Stage 3 amber locked and committed.
- **Checkpoint `0dacab4`** — "Checkpoint: relaxed particle distribution replaces raw Fibonacci lattice" — Stage 4's particle-distribution fix, locked and committed.
- **Checkpoint (this session, see `git log`)** — "Checkpoint: production luminosity — frontBoost locked at 1.30" — Stage 4's `frontBoost` calibration, locked and committed. **Stage 4 is fully complete as of this commit.**
- If anything needs to be reverted, the frontBoost-lock commit above is the last known-good committed point.
- **Important framing carried forward from the project owner explicitly:** locking `frontBoost=1.30` is a *luminosity/visibility* checkpoint only — it does **not** mean the Core's visual appearance now matches Reference 1. The remaining gap is characterized as **energy composition** (how cyan/violet/amber are structurally distributed across the membrane), not brightness — see the Stage 5 diagnosis below.
...
### STAGE 4 — whole-Core production-scale visual presence (IN PROGRESS)

**Root-cause diagnosis (confirmed via isolated A/B testing, one variable changed at a time, all others held identical):** of the hypothesized causes (particle size, `rearDarken`/`frontBoost`, `bloomIntensity`, `bloomRadius`), **`frontBoost` is overwhelmingly the dominant lever.** `depthFactor = mix(uRearDarken, uFrontBoost, pow(facing01, 2.2))` multiplies every particle's color by this factor based on how directly it faces the camera; at the committed `frontBoost = 1.0`, the front-facing majority of the visible hemisphere — the part that actually carries the "energized particle membrane" read — gets no boost at all over its raw (deliberately dark-calibrated) ramp color, so the Core reads as barely-there at normal dashboard scale. Isolated test: `frontBoost` 1.0→1.5 alone produced a 69× increase in clearly-bright pixels; `rearDarken`, `bloomIntensity`, and `bloomRadius` each produced negligible-to-no change in isolation (bloom in particular has almost nothing to amplify, since so little in a typical frame crosses the locked `bloomThreshold=0.83`). **Do not pursue rearDarken/bloomIntensity/bloomRadius/particleSize changes for this problem — evidence does not support them.**

**Distribution artifact found and FIXED while calibrating frontBoost (LOCKED, committed at `0dacab4`):** raising `frontBoost` to test values exposed a second, more fundamental problem — the raw Fibonacci sphere (`buildFibonacciSphere()`, still present in the file but **no longer used in production**) places every particle at an exactly regular golden-angle position with zero positional perturbation anywhere in the pipeline. The existing `sizeJitter`/`vOpacityJitter` only change how a particle is *drawn*, never *where* it sits, so a perfectly regular diagonal spiral/lattice weave was always latent in the geometry and only became visible once `frontBoost` restored production-scale brightness. Confirmed unambiguously via a matched mono-diagnostic (`uMonoDebug`) A/B, same crop, same `frontBoost`: the raw lattice showed an unmistakable diagonal weave; the corrected distribution showed genuinely organic, irregular coverage. **Fix, now production:** `buildFibonacciSphereRelaxed()` — a deterministic, per-particle tangent-plane offset (hash of the particle's own index, so position is fixed forever — no per-frame randomness, no flicker/crawl) of ~0.35× the average nearest-neighbor spacing, applied to each base direction and renormalized onto the unit sphere (offset is constructed orthogonal to the particle's own normal, so the perturbation is angular/tangential by construction — only a negligible second-order radial component). `particleCount`, particle size, displacement architecture, and all color/energy systems were untouched — only the 18,000 base direction vectors (computed once at mount) changed. **Do not revert to `buildFibonacciSphere()` — it is superseded and must not be restored.** Do not increase the jitter magnitude further without new evidence — 0.35× was chosen specifically to remove the regular-pattern read without turning the shell into a random cloud, and is itself now a locked value.

**`frontBoost` — LOCKED at 1.30 (Stage 4 complete):** with the distribution fixed, a narrow re-calibration (candidates 1.18/1.22/1.26/1.30/1.34, all using the approved relaxed distribution) was captured at true dashboard scale for visual review. Project owner's pick: **1.30**, with explicit reasoning — 1.18/1.22 stayed too visually recessed for the Core's role as the dashboard's central object; 1.34 started giving too much visual weight to the broad blue membrane itself; 1.30 was the balance point (shell/surface/depth clearly readable without uniform brightness). **Do not change `frontBoost` again without new project-owner authorization or a fresh evidence-gathering pass.**

### STAGE 4 IS FROZEN. Full locked-value table (supersedes the Stage 3-only table above — this is now the complete set):
| Constant | Value | Location |
|---|---|---|
| `GAMMA_CYAN` | 1.9 | `rampEnergy()`, `t>0` branch |
| `GAMMA_AMBER` | 2.2 | `rampEnergy()`, `t<=0` branch |
...
| Amber `eff` thresholds | `0.01/0.025/0.06` | `coreColorRamp()`, amber branch |
| `bloomThreshold` | 0.83 | `DEFAULT_PARAMS` |
| Particle distribution | `buildFibonacciSphereRelaxed()`, ~0.35× spacing tangent jitter | replaces `buildFibonacciSphere()` |
| `particleCount` | ~18,000 | unchanged throughout |
| `particleSize` | 0.045 | unchanged throughout, still locked at Checkpoint G |
| `rearDarken` | 0.045 | unchanged throughout |
| **`frontBoost`** | **1.30** | `DEFAULT_PARAMS` — Stage 4's own deliverable |

Geometry/displacement architecture (Fold #1/#2, warp), camera (`position: [0,0,4.8]`, `fov: 50`), and animation/state behavior are all unchanged from Phase 1-7 and were explicitly held locked throughout Stage 4's diagnosis.

### STAGE 5 — Reference 1 energy composition (diagnosis-only, NOT implemented)
Explicit framing from the project owner: locking Stage 4's luminosity does **not** mean the Core matches Reference 1 — the remaining gap is **energy composition**, not brightness. Current cool-energy (cyan/violet) architecture reads as sparse/isolated accents rather than Reference 1's richer, structurally-coherent ridges/pathways/branching structures across the membrane. A diagnosis-only pass (inspecting `energyMask()`/`rampEnergy()`/`coreColorRamp()`'s cyan branch — the same architecture the locked amber system reuses with a phase offset) was completed and presented to the project owner at the end of this session, covering: how cyan/violet placement is currently determined, why it reads sparse/localized, which variables control WHERE vs. HOW MUCH AREA vs. INTENSITY PROGRESSION, whether the existing `energyMask()` field is structurally capable of producing Reference 1's ridge/pathway character without replacing the architecture, and a recommended smallest correction. **None of this diagnosis has been implemented.** Whoever continues: read the conversation record for the full diagnosis (not reproduced in this doc), or re-derive it directly from the shader — do not skip straight to implementation without the project owner's explicit approval of a specific correction, per this project's established pattern. Explicitly OUT of scope for this stage per the project owner: do not raise `frontBoost` further, do not add global bloom, do not increase particle size/count, do not repaint large cyan/amber regions, do not alter geometry or the approved particle distribution, do not touch the locked amber gating architecture (may only be *revealed* more, never redesigned).
...
**Read §0 first — it is more current than everything below it.** Stage 3 (amber) and Stage 4 (whole-Core luminosity: particle distribution + `frontBoost=1.30`) are **both fully approved, locked, and committed** — do not reopen either. The open item is **Stage 5 (Reference 1 energy composition)**, currently at diagnosis-only status — a diagnosis was presented but **nothing has been implemented**.

1. Read this entire document, starting with §0.
2. Run `git log --oneline -6` and `git status`/`git diff` to establish exactly what's committed. Expected chain (oldest→newest) as of this writing: `5b94212` (Stage 2 colorspace) → `178e673` (Stage 3 amber) → `0dacab4` (Stage 4 particle distribution) → a commit locking `frontBoost=1.30` (Stage 4 complete). If `git log` shows anything past that, read its message first — it may mean Stage 5 work has already started.
3. Inspect `components/JarvisCore3D.tsx` directly — do not trust this document's claims about the shader's current state without verifying them against actual source. Confirm `DEFAULT_PARAMS.frontBoost` reads `1.3` and the locked-value table in §0 still matches.
4. **Amber is DONE.** Do not touch `GAMMA_AMBER`, the amber ridge-gate architecture, the `0.006` off-ridge floor, `RIDGE_MIN` (0.045), or the amber thresholds (`0.01/0.025/0.06`) unless a future change accidentally breaks its topology (0 off-ridge leak, 100% genuine on-ridge visibility) — if that happens, diagnose with the same same-instant/multi-frame evidence methodology before changing anything.
5. **The particle distribution fix is DONE.** Do not revert to `buildFibonacciSphere()` (still present in the file, unused) and do not increase `buildFibonacciSphereRelaxed()`'s jitter magnitude (~0.35× spacing) without new evidence.
6. **`frontBoost` is DONE, locked at 1.30.** Do not change it without new project-owner authorization.
7. The open item is **Stage 5 — cool-energy (cyan/violet) composition** to bring the Core closer to Reference 1's richer structural hierarchy (branching ridges/creases/pathways vs. the current sparse isolated-accent read). A diagnosis of the existing `energyMask()`/`rampEnergy()`/`coreColorRamp()` cyan-branch architecture was completed and presented to the project owner at the end of the prior session but is **not reproduced in this document** — read the conversation record, or re-derive the diagnosis directly from the shader before proposing anything. **Do not implement any correction without the project owner's explicit approval of a specific plan.** Explicitly out of scope per the project owner: raising `frontBoost` further, global bloom increases, particle size/count changes, large cyan/amber repaints, geometry or distribution changes, redesigning the locked amber gating architecture.
8. Do **not** start Phase 8 ("Performance, QA & Final Polish") or the general Phase 8 pre-implementation inspection (§14/§15) unless the project owner explicitly asks for it in this new conversation.
9. Do not let any of this quietly expand into violet-specific tuning beyond what Stage 5 authorizes, general color grading unrelated to energy composition, motion/behavior work, or "Thinking Orb" integration — none of the latter two are authorized yet.
10. As always in this project: **stop and wait for the project owner's explicit approval before writing any implementation code.**
